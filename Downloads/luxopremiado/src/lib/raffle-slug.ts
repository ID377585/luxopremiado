...
const LEGACY_RAFFLE_SLUG_ALIASES = new Set(["luxo-premiado"]);
...
export function canonicalizeRaffleSlug(value: string | undefined | null): string | null {
...
export function getDefaultRaffleSlug(): string {
  return canonicalizeRaffleSlug(process.env.NEXT_PUBLIC_DEFAULT_RAFFLE_SLUG) ?? FALLBACK_RAFFLE_SLUG;
}
...
export function isDefaultRaffleSlug(slug: string): boolean {
  ...
  return normalized === defaultSlug || (defaultSlug === FALLBACK_RAFFLE_SLUG && LEGACY_RAFFLE_SLUG_ALIASES.has(normalized));
}

export function buildLandingPathForSlug(slug: string, anchor?: string): string {
  const normalizedSlug = canonicalizeRaffleSlug(slug) ?? getDefaultRaffleSlug();
