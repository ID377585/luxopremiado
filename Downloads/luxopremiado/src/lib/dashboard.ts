...
interface OrderAffiliateWithOrderRow {
  status: string | null;
...
function createEmptyVipNetworkMetrics(): VipNetworkMetrics {
  return {
...
export async function getVipNetworkMetrics(userId: string): Promise<VipNetworkMetrics> {
  if (!hasSupabaseEnv() || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return createEmptyVipNetworkMetrics();
  }
  ...
  try {
    ...
    const rows = ((data ?? []) as OrderAffiliateWithOrderRow[]) ?? [];

    const approvedCommissionCents = rows
      .filter((row) => row.status === "approved" || row.status === "paid")
      .reduce((sum, row) => sum + asNumber(row.commission_cents), 0);
    ...
    for (const row of rows) {
      ...
      partnerSpendByUser.set(
        order.user_id,
        (partnerSpendByUser.get(order.user_id) ?? 0) + asNumber(order.amount_cents),
      );
    }
...
> {
  ...
  try {
    ...
    const orderMap = new Map(
      ((orders ?? []) as OrderAmountRow[]).map((order) => [order.id, order.amount_cents]),
    );
    ...
    return paymentRows.map((payment) => ({
      ...
      order: orderMap.has(payment.order_id)
        ? { amount_cents: orderMap.get(payment.order_id)! }
        : null,
    }));
  } catch {
    return [];
...
export async function getMyProfile(userId: string): Promise<DashboardProfile> {
  ...
  try {
    ...
    return {
      ...
      vip_points: asNumber(data?.vip_points),
  ...
  try {
    ...
    const ranges = (() => {
      ...
      return prizeConfigs.map((p) => {
        ...
        return {
          order: Number(p.prize_order ?? 0),
          label: p.prize_label as string,
          start,
          end,
        };
      });
    ...
    const filteredData = raffleId
      ? (data ?? []).filter((row) => row.raffle_id === raffleId)
      : data ?? [];
...
export async function getMyAffiliate(userId: string): Promise<DashboardAffiliate | null> {
  ...
  try {
    ...
    const approvedCommissionCents =
      referrals
        ?.filter((row) => row.status === "approved" || row.status === "paid")
        .reduce((acc, row) => acc + asNumber(row.commission_cents), 0) ?? 0;
...
export async function getMyVipStatus(userId: string, email?: string | null): Promise<DashboardVipStatus> {
  ...
  try {
    ...
    const raffleSpendCents =
      ordersResult.data?.reduce((sum, row) => sum + asNumber(row.amount_cents), 0) ?? 0;

    const auctionSpendCents =
      auctionWinsResult.data?.reduce((sum, row) => sum + asNumber(row.winner_bid_cents), 0) ?? 0;
    ...
    if (isAdmin) {
      const points = Math.max(asNumber(profileResult.data?.vip_points), pointBreakdown.automaticPoints);
    ...
    const resolved = resolveVipAccess({
      ...
      persistedPoints: asNumber(profileResult.data?.vip_points),
      raffleSpendCents,
      auctionSpendCents,
      partnerInvestmentCents: networkMetrics.partnerInvestmentCents,
    ...
    const xp = calculateVipXpProgress(0);
    ...
    return {
      ...
      manual_override: false,
      unlocked_at: null,
      locked_reason: "Não foi possível carregar o status VIP no momento.",
      progress_percent: 0,
      remaining_points: 8000,
      next_tier: "vip",
      next_tier_label: "VIP",
      next_tier_min_points: 8000,
      metrics: {
        raffle_spend_cents: 0,
        auction_spend_cents: 0,
        network_investment_cents: 0,
        approved_commission_cents: 0,
        referred_orders: 0,
        qualified_partners_for_vip: 0,
        qualified_partners_for_elite: 0,
      },
      point_breakdown: {
        raffle_points: 0,
...
export async function getDefaultAffiliateRaffleSlug(): Promise<string> {
  ...
  try {
    return await resolveAvailableRaffleSlug();
  } catch {
    return "bigode-das-rifas";
  }
}